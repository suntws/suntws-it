...
{
    /// <summary>
    /// Summary description for uploadstencilimage
    /// </summary>
    public class uploadstencilimage : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        public void ProcessRequest(HttpContext context)
        {
            try
            {
                ...
                {
                    if (!context.Request.UrlReferrer.LocalPath.Contains("claimregister1.aspx"))
                    {
                        context.Response.ContentType = "text/plain";
                        string strClaimCode = context.Session["ClaimCustCode"].ToString();
                        string strClaimNo = context.Session["ClaimNo"].ToString();
                        string strClaimStencil = context.Session["ClaimStencil"].ToString();
                        string serverURL = HttpContext.Current.Server.MapPath("~/").Replace("SCOTS", "TTS").Replace("COTS", "TTS");
                        if (!Directory.Exists(serverURL + "/claimimages/" + strClaimCode + "/" + strClaimNo + "/" + strClaimStencil + "/"))
                            Directory.CreateDirectory(serverURL + "/claimimages/" + strClaimCode + "/" + strClaimNo + "/" + strClaimStencil + "/");
                        string path = serverURL + "/claimimages/" + strClaimCode + "/" + strClaimNo + "/" + strClaimStencil + "/";
                        for (int z = 0; z < context.Request.Files.Count; z++)
                        {
                            ...
                            if (file1.Exists)
                            {
                                Directory.Move(strsave, serverURL + "/claimimages/" + strClaimCode + "/" + strClaimNo + "/" + strClaimStencil + "/A.AdditionalStencil_" + strName.ToString().Replace("(", "").Replace(")", "").Replace("&", "").Replace(":", "").Replace("/", ""));
                            }
                        }
                    }
                    else
                    {
                        ...
                        for (int z = 0; z < context.Request.Files.Count; z++)
                        {
                            ...
                            if (file1.Exists)
                            {
                                Directory.Move(strsave, serverURL + "/claimimages/" + context.Session["cotscode"].ToString() + "/" + DateTime.Now.ToString("dd-MM-yyyy") + "/dummy/A.Stencil_" + strName.ToString().Replace("(", "").Replace(")", "").Replace("&", "").Replace(":", "").Replace("/", ""));
                            }
                        }
                    }
                }
            ...
            {
                Utilities.WriteToErrorLog("TTS", context.Request.Url.ToString(), MethodBase.GetCurrentMethod().Name, 1, ex.Message);
            }
        }
