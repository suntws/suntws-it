...
namespace GT
{
    public partial class frmProdPlan : Form
    {
        ...
        private void frmProdPlan_Load(object sender, EventArgs e)
        {
            try
            {
                lblMouldMsg.Text = "";
                ...
                DataTable dtPressList = (DataTable)dba.ExecuteReader_SP("sp_sel_PressList_SizeWise_v1", DBAccess.Return_Type.DataTable);
        ...
        private void cboPress_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (cboPress.SelectedIndex > 0)
                {
                    ...
                    dtItems = (DataTable)dba.ExecuteReader_SP("sp_sel_Items_For_ProdAllocation_v2", spSel, DBAccess.Return_Type.DataTable);
        ...
        private void cboRim_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    bind_ProductionPlan_SizeWise(cboSize.SelectedValue.ToString(), cboRim.SelectedValue.ToString());
        ...
        private void bind_ProductionPlan_SizeWise(string strSize, string strRim)
        {
            try
            {
                ...
                DataTable dtPlan = (DataTable)dba.ExecuteReader_SP("sp_sel_ProductionPlan_SizeWise", spList, DBAccess.Return_Type.DataTable);
                if (dtPlan.Rows.Count > 0)
                {
                    ...
                    dgv_AllocateList.Columns[6].ReadOnly = true;
                    dgv_AllocateList.Columns[6].HeaderText = "REMAIN QTY";
                    dgv_AllocateList.Columns[6].Width = 60;

                    SqlParameter[] spCount = new SqlParameter[] { new SqlParameter("@tyresize", strSize), new SqlParameter("@rimsize", strRim) };
                    int intMouldCount = (int)dba.ExecuteScalar_SP("sp_sel_MouldCount_sizeWise", spCount);
                    lblMouldMsg.Text = "TOTAL MOULD'S " + intMouldCount.ToString() + " AVAILABLE MOULD'S " + (intMouldCount - dtPlan.Rows.Count).ToString();
                }
            }
            catch (Exception ex)
            {
                ...
                if (e.RowIndex >= 0 && e.ColumnIndex == 0)
                {
                    ...
                    if (row.Cells["checkBoxColumn"].Value != null && Convert.ToBoolean(row.Cells["checkBoxColumn"].Value))
                    {
                        ...
                        if (dgv_AllocateList.Rows.Count <= 0)
                            bind_ProductionPlan_SizeWise(row.Cells[5].Value.ToString(), row.Cells[6].Value.ToString());
                        ...
                        if (dtPress.Rows.Count > 0)
                        {
                            ...
                            foreach (DataGridViewRow gRow in dgv_AvalPress.Rows)
                            {
                                if (gRow.Cells[0].Value.ToString() == cboPress.SelectedValue.ToString())
                                {
                                    ...
                                    lblRemainSlot.Text = (Convert.ToInt32(gRow.Cells[1].Value) - Convert.ToInt32(gRow.Cells[2].Value)).ToString();
                                    ...
                                    DataTable dtMould = (DataTable)dba.ExecuteReader_SP("sp_sel_mouldcode_for_allocation_v2", sp0, DBAccess.Return_Type.DataTable);
