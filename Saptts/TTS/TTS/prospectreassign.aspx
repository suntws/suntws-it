<%@ Page Title="" Language="C#" MasterPageFile="~/master.Master" AutoEventWireup="true"
    CodeBehind="prospectReassign.aspx.cs" Inherits="TTS.prospectReassign" EnableEventValidation="false" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="HeadContent" runat="server">
    ...
    <style type="text/css">
        ...
        .ui-state-highlight1, .ui-widget-content .ui-state-highlight1, .ui-widget-header .ui-state-highlight1
        {
            border: 1px solid #fcd113;
            background: #25B1D2 !important;
            color: #0A0A0A;
        }
    </style>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
                ...
                <td>
                    <table cellspacing="0" rules="all" border="1" class="tableCust">
                        ...
                        <tr>
                            ...
                            <td>
                                <asp:DropDownList ID="ddlPort" runat="server" Width="100px" CssClass="ddlchange"
                                    ClientIDMode="Static">
                                </asp:DropDownList>
                            </td>
                            <td>
                                <asp:DropDownList runat="server" ID="ddlLeadList" ClientIDMode="Static" Width="170px"
                                    CssClass="ddlchange">
                                </asp:DropDownList>
                            </td>
                        </tr>
                        ...
                        <table>
                            ...
                            <tr>
                                <td style="padding-top: 5px;">
                                    <div style="width: 1070px; float: left; background-color: #ccc; display: none;" id="divassignuser">
                                        <table class="tableCust">
                                            ...
                                            <tr>
                                                <td style="text-align: center; padding-top: 3px;">
                                                    <asp:Button runat="server" ID="btnReAssign" Text="ASSIGN" OnClientClick="javascript:return crtlRaAssignValidation();"
                                                        OnClick="btnReAssign_Click" CssClass="btnsave" />
    ...
    <asp:HiddenField runat="server" ID="hdnLeadStatus" ClientIDMode="Static" Value="" />
    <asp:HiddenField runat="server" ID="hdncustcode" ClientIDMode="Static" Value="" />
    <script src="Scripts/grid.locale-en.js" type="text/javascript"></script>
    <script src="Scripts/jquery.jqGrid.min.js" type="text/javascript"></script>
    <script type="text/javascript">
        $(document).ready(function () {
            var supplierhistory; var LeadStatus; var left;
            ...
            $('body').click(function (e) {
                if (e.target.className != "cssTip" && e.target.className != 'supplier' && e.target.className != 'LeadStatus' && e.target.id != "myToolTip1") $('#myToolTip').hide();
            });
            $('#ddlCountryList').change(function () {
                ...
                } GetCustomers($("#ddlCustList option:selected").text(), $("#ddlCountryList option:selected").text(), 'ALL', $("#ddlFlagList option:selected").text(), $("#ddlPort option:selected").text(), $("#ddlFocusList option:selected").text(), $("#ddlLeadList option:selected").text(), $("#ddlSupplier option:selected").text(), $("#ddlLeadSource option:selected").text());
            });
            $('.ddlchange').change(function () { GetCustomers($("#ddlCustList option:selected").text(), $("#ddlCountryList option:selected").text(), $("#ddlCity option:selected").text(), $("#ddlFlagList option:selected").text(), $("#ddlPort option:selected").text(), $("#ddlFocusList option:selected").text(), $("#ddlLeadList option:selected").text(), $("#ddlSupplier option:selected").text(), $("#ddlLeadSource option:selected").text()); });
        ...
        function gridbind() { GetCustomers($("#ddlCustList option:selected").text(), $("#ddlCountryList option:selected").text(), $("#ddlCity option:selected").text(), $("#ddlFlagList option:selected").text(), $("#ddlPort option:selected").text(), $("#ddlFocusList option:selected").text(), $("#ddlLeadList option:selected").text(), $("#ddlSupplier option:selected").text(), $("#ddlLeadSource option:selected").text()); }

        function GetCustomers(CustName, country, City, flag, port, focus, Lead, Supplier, LeadSource) {
            supplierhistory = ''; LeadStatus = '';
            var focus1 = encodeURIComponent(focus);
            ...
            if (supplierhistory != '' && LeadStatus != '') {
                $.ajax({
                    type: "POST",
                    url: 'BindRecords.aspx?type=GetAssignList&CustName=' + CustName + '&country=' + country1 + '&City=' + City1 + '&flag=' + flag + '&port=' + port + '&focus=' + focus1 + '&Lead=' + Lead + '&Supplier=' + Supplier + '&LeadSource=' + LeadSource + '',
                    ...
                    success: function OnSuccess(response) {
                        ...
                        $("#jqGrid").jqGrid({
                            ...
                            colNames: ['<input type="checkbox" name="chck" id="chkall" class="cbox" onclick="SelectSingle1(this);" />', 'CODE', 'CUSTOMER NAME', 'CITY', 'COUNTRY', 'PORT', 'LEAD SOURCE', 'FOCUS', 'FLAG', 'LEAD', 'SUPERVISOR', 'MANAGER', 'SUPPLIER', 'STATUS'],
                            colModel: [
                     { name: 'select', label: 'select', align: 'center', width: 20, formatter: Checkbtn, sortable: false, resizable: false },
                    { name: 'custcode', key: true, width: 50, resizable: false },
                    { name: 'Custname', width: 200, resizable: false },
                    ...
                    { name: 'assignto', width: 90, resizable: false },
                    { name: 'Supervisor', width: 90, resizable: false },
                    { name: 'Manager', width: 90, resizable: false },
                    { name: 'Supplieraction', sortable: false, formatter: SupplierData, width: 60, align: 'center' },
                    { name: 'Leadaction', sortable: false, formatter: LeadData, width: 45, align: 'center' }
                ],
                            viewrecords: true, height: 'auto', width: 1060, rowNum: 20, rowList: [20, 30, 50, 100], pager: "#jqGridPager",
                            gridComplete: function (e) {
                                $('#divcheck').find($('.cbox')).click(function () { showLead(); });
                                $('.cbox').parent().removeClass('ui-jqgrid-sortable');
                                ShowToolTip();
                                showLead();
                            }
                        });
        ...
        function crtlRaAssignValidation() {
